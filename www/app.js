...
async function init() {
    // Check login state
    if (localStorage.getItem('fortune_logged_in') === 'true') {
        showApp();
    }
...
function handleLogin() {
    ...
    if (user === 'dink' && pass === '1223') {
        localStorage.setItem('fortune_logged_in', 'true');
        showApp();
    } else {
        UI.loginError.textContent = state.lang === 'zh' ? "憑證無效" : "Invalid credentials";
        UI.passwordInput.value = "";
    }
}

function showApp() {
    UI.loginSection.classList.remove('active');
    setTimeout(() => {
        UI.loginSection.style.display = 'none';
        UI.appContent.style.display = 'block';
        UI.inputSection.classList.add('active');
    }, 500);
}
